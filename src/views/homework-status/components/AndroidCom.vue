<template lang="">
    <div class="android-homework-status">
         <virtual-list
          :containerWidth="120"
          :size="10"
          :list="androidPersonList"
          :itemHeight="45"
          title=""
          :containerHeight="470"
        >
...
export default defineComponent({
  ...
  setup() {
    const androidPersonList = ref<Student[]>([]);
    onMounted(async () => {
      // 这里要传入方向的ID，也就是 1
      const result = await getHomeworkStatus(4);
      androidPersonList.value = computed(() => result.data.users).value;
      const arr = calculateAverageScore(androidPersonList.value)
      const [middle, average, range, quartile, variance, deviation] = computedStatistics(arr)
      const chartDom = document.getElementById("android") as HTMLDivElement
