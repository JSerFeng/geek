...
export default defineComponent({
  ...
  setup() {
    const pythonPersonList = ref<Student[]>([]);
    onMounted(async () => {
      // 这里要传入方向的ID，也就是 1
      const result = await getHomeworkStatus(3);
      pythonPersonList.value = computed(() => result.data.users).value;
      const arr = calculateAverageScore(pythonPersonList.value)
      const [middle, average, range, quartile, variance, deviation] = computedStatistics(arr)
      const chartDom = document.getElementById("python") as HTMLDivElement
