...
export default defineComponent({
  ...
  setup() {
    const endPersonList = ref<Student[]>([]);
    onMounted(async () => {
      // 这里要传入方向的ID，也就是 1
      const result = await getHomeworkStatus(2);
      endPersonList.value = computed(() => result.data.users).value;
      const arr = calculateAverageScore(endPersonList.value)
      const [middle, average, range, quartile, variance, deviation] = computedStatistics(arr)
      const chartDom = document.getElementById("end") as HTMLDivElement
