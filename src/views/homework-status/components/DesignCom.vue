<template lang="">
    <div class="design-homework-status">
         <virtual-list
          :containerWidth="120"
          :size="10"
          :list="designPersonList"
          :itemHeight="45"
          title=""
          :containerHeight="470"
        >
...
export default defineComponent({
  ...
  setup() {
    const designPersonList = ref<Student[]>([]);
    onMounted(async () => {
      // 这里要传入方向的ID，也就是 1
      const result = await getHomeworkStatus(5);
      console.log(result)
      designPersonList.value = computed(() => result.data.users).value;
      const arr = calculateAverageScore(designPersonList.value)
      const [middle, average, range, quartile, variance, deviation] = computedStatistics(arr)
      const chartDom = document.getElementById("design") as HTMLDivElement
